...
<style>
  ...
  .card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 2px 8px rgba(15,23,42,.04)}
  .input{border:1px solid #e5e7eb;border-radius:12px;padding:.6rem .9rem;outline:none;width:100%}
...
function del(id, pr){
  Swal.fire({
    ...
    fd.append('id', id);
    try{
      const r = await fetch('delete_pr.php', {method:'POST', body:fd});
      const j = await r.json();
      if(j.ok){
        Swal.fire({icon:'success',title:'Terhapus'}).then(()=> fetchList());
      }else{
        Swal.fire({icon:'error',title:'Gagal', text: j.msg || 'Tidak diketahui'});
      }
    }catch(e){
